...
void dfs(vector<string> &a, vector<vector<ll>> &vis, int i, int j, ll n, ll m, bool &st, ll &cur)
{
    //  cout<<"\n"<<n<<"\n";
    ...
    if (!vis[i - 1][j] && a[i - 1][j] == '.')
        dfs(a, vis, i - 1, j, n, m, st, cur);
    if (!vis[i + 1][j] && a[i + 1][j] == '.')
        dfs(a, vis, i + 1, j, n, m, st, cur);
    if (!vis[i][j - 1] && a[i][j - 1] == '.')
        dfs(a, vis, i, j - 1, n, m, st, cur);
    if (!vis[i][j + 1] && a[i][j + 1] == '.')
        dfs(a, vis, i, j + 1, n, m, st, cur);
}
